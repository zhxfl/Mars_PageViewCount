...
int main( int argc, char** argv) 
{
	...
	//------------------------------------------------------------------
	checkCudaErrors(cudaMemcpy(spec->inputKeys, spec->outputKeys, spec->outputAllKeySize, cudaMemcpyDeviceToHost));
	checkCudaErrors(cudaMemcpy(spec->inputVals, spec->outputVals, spec->outputAllValSize, cudaMemcpyDeviceToHost));
	checkCudaErrors(cudaMemcpy(spec->inputOffsetSizes, spec->outputOffsetSizes, spec->outputRecordCount * sizeof(int4), cudaMemcpyDeviceToHost));
	spec->inputRecordCount = spec->outputRecordCount;
	checkCudaErrors(cudaFree(spec->outputKeys));
	checkCudaErrors(cudaFree(spec->outputVals));
	checkCudaErrors(cudaFree(spec->outputOffsetSizes));
	...
	//------------------------------------------------------------------
#ifdef __OUTPUT__
	validate(spec, h_filebuf, d_filebuf, fileSize, 10);
#endif
